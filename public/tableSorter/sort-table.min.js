...
function sortTable(a, b, c) {
  ...
  null != d &&
    ((sortTable.sortCol = d[0].replace(/js-sort-/, "")),
    (a.className = a.className.replace(new RegExp(" ?" + d[0] + "\\b"), "")));
  "undefined" === typeof b && (b = sortTable.sortCol);
  "undefined" !== typeof c
    ? (sortTable.sortDir = -1 == c || "desc" == c ? -1 : 1)
    : ((d = a.className.match(/js-sort-(a|de)sc/)),
      (sortTable.sortDir =
        null != d && sortTable.sortCol == b
          ? "js-sort-asc" == d[0]
            ? -1
            : 1
          : 1));
  a.className = a.className.replace(/ ?js-sort-(a|de)sc/g, "");
  a.className += " js-sort-" + b;
  sortTable.sortCol = b;
  a.className += " js-sort-" + (-1 == sortTable.sortDir ? "desc" : "asc");
  b < a.tHead.rows[a.tHead.rows.length - 1].cells.length &&
    (d = a.tHead.rows[a.tHead.rows.length - 1].cells[b].className.match(
      /js-sort-[-\w]+/
    ));
  for (c = 0; c < a.tHead.rows[a.tHead.rows.length - 1].cells.length; c++)
    b ==
      a.tHead.rows[a.tHead.rows.length - 1].cells[c].getAttribute(
        "data-js-sort-colNum"
      ) &&
      (d = a.tHead.rows[a.tHead.rows.length - 1].cells[c].className.match(
        /js-sort-[-\w]+/
      ));
  sortTable.sortFunc = null != d ? d[0].replace(/js-sort-/, "") : "string";
  a.querySelectorAll(".js-sort-active").forEach(function (a) {
    a.className = a.className.replace(/ ?js-sort-active\b/, "");
  });
  a.querySelectorAll('[data-js-sort-colNum="' + b + '"]:not(:empty)').forEach(
    function (a) {
  ...
  for (
    "none" != sortTable.sortFunc && b.sort(sortTable.compareRow);
    a.firstChild;
...
sortTable.compareRow = function (a, b) {
  "function" != typeof sortTable[sortTable.sortFunc] &&
...
sortTable.date = function (a) {
  return "undefined" !== typeof okDate
    ? (a = okDate(sortTable.stripTags(a.innerHTML)))
      ? a.getTime()
      : 0
    : new Date(sortTable.stripTags(a.innerHTML)).getTime() || 0;
};
...
sortTable.input = function (a) {
  for (var b = 0; b < a.children.length; b++)
    if (
      "object" == typeof a.children[b] &&
      "undefined" != typeof a.children[b].value
    )
      return a.children[b].value.toLowerCase();
...
sortTable.init = function () {
  var a = document.querySelectorAll
    ? document.querySelectorAll("table.js-sort-table")
    : document.getElementsByTagName("table");
  for (var b = 0; b < a.length; b++)
    if (
      (document.querySelectorAll ||
        null !== a[b].className.match(/\bjs-sort-table\b/)) &&
      !a[b].attributes["data-js-sort-table"]
    ) {
      if (a[b].tHead) var c = a[b].tHead;
      else
        (c = document.createElement("thead")),
          c.appendChild(a[b].rows[0]),
          a[b].insertBefore(c, a[b].children[0]);
      for (var d = 0; d < c.rows.length; d++)
        for (var e = 0, f = 0; e < c.rows[d].cells.length; e++)
          if (!c.rows[d].cells[e].className.match(/\bjs-sort-none\b/)) {
            c.rows[d].cells[e].setAttribute("data-js-sort-colNum", f);
            var g = sortTable.getClickHandler(a[b], f);
            window.addEventListener
              ? c.rows[d].cells[e].addEventListener("click", g)
              : window.attachEvent &&
                c.rows[d].cells[e].attachEvent("onclick", g);
...
window.addEventListener
  ? window.addEventListener("load", sortTable.init, !1)
